...
export interface ImageHistorySummary<T extends NinaImageHistoryEntry = NinaImageHistoryEntry> {
  readonly latestImage: T | null;
...
export function summarizeImageHistory<T extends NinaImageHistoryEntry>(
  entries: ReadonlyArray<T>,
): ImageHistorySummary<T>;
export function summarizeImageHistory<T extends NinaImageHistoryEntry>(
  entries: ReadonlyArray<T>,
): ImageHistorySummary<T> {
  if (!entries.length) {
