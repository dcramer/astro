...
import { ninaSessionHistorySchema, ninaSessionListResponseSchema } from "./session-schemas";
...
export interface NinaStretchOptions {
  readonly autoStretchFactor: number;
  readonly blackClipping: number;
  readonly unlinkedStretch: boolean;
}
...
export interface NinaSessionHistory {
  ...
  readonly stretchOptions?: NinaStretchOptions | null;
...
async function fetchJson<T>(baseUrl: string, path: string, schema?: ZodType<T>): Promise<T | null> {
  ...
  try {
    ...
    if (!parsed.success) {
      if (shouldLogSessionFailure(url)) {
        console.warn(`Session API response from ${url} did not match the expected schema`, parsed.error.flatten());
      }
      return null;
    }
...
export async function fetchSessionList(
  ...
  return fetchJson<NinaSessionListResponse>(
    baseUrl,
    "/sessions/sessions.json",
    ninaSessionListResponseSchema,
  );
}
...
export async function fetchSessionHistory(
  ...
  return fetchJson<NinaSessionHistory>(
    baseUrl,
    `/sessions/${sessionKey}/sessionHistory.json`,
    ninaSessionHistorySchema,
  );
}
